...
# Given a binary tree, remove the nodes in which there 
# is only 1 child, so that the binary tree is a full binary tree.
...
def fullBinaryTree(node):
  ...
  c = ''
  if node.left is not None: c += 'l'
  if node.right is not None: c += 'r'
